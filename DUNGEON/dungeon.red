...
; walls alternate being dark and light so that a light and dark wall do not
; directly abut each other.  For this rule to be possible, if you look on one
; side at a wall and then walk around to the other side and look at that same
; edge, it will be the opposite shading.
;
shading-for-wall: function [
    {Get shading for wall (as WORD! index into SOLID-CHAR) for wall}
...
in-bounds: function [
    {Limit coordinate pair to boundary}

    pos [block!]
    low [block!]
    high [block!]

    return: {true if no limit needed, false if limited} [logic!]
] [
    all-inside: true
    case/all [
        pos/1 < low/1 [
...
; 1. When dungeon.red was initially written, you could index into an array
;    using decimal numbers, e.g. block/2.0 was the same as block/2 - but in
;    Red 0.6.5 you get #[none] back for indexing with decimals.  Must round,
;    and unfortunately ROUND does not turn decimal numbers into integers
;    unless you use ROUND/TO and specify an integer.  :-(
;
; 2. In defiance of the laws of perspective, walls at the same distance are
;    shown at the same width for as far as they can be seen to the left and
;    right within the screen boundaries.
;
draw-flat-wall: function [
    {Draw a flat wall at the given depth}

    buffer [block!] "Display buffer to draw into"
    depth [integer!] "How many steps in the distance the wall is"
    x-offset [integer!] "Steps off center to draw (- is left, + is right)"
    shading [word!] "Word index into SOLID-CHAR for shading"

    return: [logic!] "Did flat wall fit completely in the display"
] [
    dims: display/flat-dims-for-depth/(depth)

    start-pos: reduce [
        round/to (  ; Red 0.6.5 requires rounding [1]
        ...
        start-pos/1 + dims/1 - 1  ; width ignores offset [2]
...
render-3d: function [
    {Print 14x13 matrix of characters approximating C64 solid blocks/triangles}
    ...
    while [depth > 0] [
        ...
        ; first to go left of center, then 1 to go right of center.  Finally
        ; we draw the center wall and slant walls at that depth.  The depths
